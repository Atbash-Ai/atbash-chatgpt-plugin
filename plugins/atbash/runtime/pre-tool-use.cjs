...
// the bundled hook next to it cannot load. A PreToolUse host lets a tool call proceed when a hook
// times out or exits without a decision (Claude Code documents this for any exit code other than 0
// or 2; Codex 0.154.0 was measured on 2026-09-17: it proceeds on a timeout, on exit 1, on exit 2 - even
// with a deny on stdout - and on exit 0 with an empty stdout; it blocks only on a deny with exit 0), so the two failures that would silently remove the
