...
const normalize = value => {
  const p = value instanceof URL ? fileURLToPath(value) : Buffer.isBuffer(value) ? value.toString() : value;
...
for (const method of ['existsSync', 'readFileSync', 'openSync', 'writeFileSync', 'appendFileSync', 'mkdirSync', 'statSync', 'lstatSync', 'realpathSync', 'unlinkSync', 'rmSync', 'chmodSync', 'readdirSync']) {
  const original = fs[method].bind(fs);
  fs[method] = (file, ...args) => original(protect(file, method === 'existsSync' || method === 'readFileSync'), ...args);
}
for (const method of ['readFile', 'open', 'writeFile', 'appendFile', 'mkdir', 'stat', 'lstat', 'realpath', 'unlink', 'rm', 'rmdir', 'chmod', 'readdir', 'access']) {
...
for (const name of ['spawn', 'spawnSync', 'exec', 'execSync', 'execFile', 'execFileSync', 'fork']) {
  childProcess[name] = () => { trace.browser++; throw Error('Process launch prohibited by test'); };
}
net.Server.prototype.listen = () => { trace.server++; throw Error('Server launch prohibited by test'); };
net.Socket.prototype.connect = () => { trace.network++; throw Error('Network prohibited by test'); };
for (const api of [http, https]) {
  api.request = api.get = () => { trace.network++; throw Error('Network prohibited by test'); };
}
...
globalThis.fetch = async (input, init) => {
  ...
  if (!['https://node0.testnet.chromia.com', 'https://node1.testnet.chromia.com'].some(node => url === `${node}/query_gtv/${rid}`) || init?.method !== 'POST') {
    trace.network++;
  ...
  trace.queries.push({ name, organization: args.org_name ?? null, publicKey: args.pubkey ? Buffer.from(args.pubkey).toString('hex') : null, url });
  const result = name === 'get_org_account_id' ? PcBuffer.alloc(32, 7)
    : name === 'get_org_agent_capacity' ? { max_agents: 1, active_count: 1, total_count: 1 } : null;
