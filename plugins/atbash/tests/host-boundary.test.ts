 ...
 *
 * Codex 0.154.0, measured on 2026-09-17: a hook that times out does not block the tool call, nor
 * does one that exits 1, exits 2 (even with a deny on stdout) or exits 0 with an empty stdout -
 * only a deny on stdout with exit 0 blocks. Claude Code additionally documents exit 2 as blocking.
 ...
 * Both were red on 0.3.3 (measured 40.8 s for a slow-but-alive judge; exit 1 with no output for
